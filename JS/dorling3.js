...
d3.json("data/topojson/LSOAden.json", function(error, lsoas) {


    var neighbors = topojson.neighbors(lsoas.objects.LSOAden.geometries);

    var land = {
        type: "GeometryCollection",
        geometries: polygons(lsoas.objects.LSOAden.geometries)//,//,
        //radius: this.geometries.radius //lsoas.objects.wales_low_soa_2001.geometries.properties.LSOA01CD
    };

    var neighbors = topojson.neighbors(land.geometries);
    //var radii = land.geometries;
    var nodes = topojson.feature(lsoas, land).features.map(function(d) {
        //exterior(d.geometry);
        ...
        return {
            ...
            oy: centroid[1],
            geometry: d.geometry,
            dorling: dorling(d.geometry, rad)
        };
    });
...
function dorling(d, density, dep) {
    switch (d.type) {
        case "Polygon": return circle(projectRing(d.coordinates[0]), density);
        case "MultiPolygon": return circle(projectRing(d.coordinates[0][0]));
    }
...
function circle(coordinates, radius) {
    var circle = [],
        length = 0,
        lengths = [length],
    ...
    var area = polygon.area(),
        radius = radius,//Math.sqrt(Math.abs(area) / Math.PI),
...
function polygons(geometries) {
    ...
    return d3.merge(geometries.map(function(geometry) {
        return (geometry.type === "MultiPolygon" ? geometry.arcs : [geometry.arcs]).map(function(d) {
            return {id: ++id, type: "Polygon", arcs: d, parent: geometry, radius: geometry.properties.KDE};
        });
    }));
}
